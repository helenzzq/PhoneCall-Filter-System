...
def helper(customer_list: List[Customer], call: Call) -> None:
    """A helper function for process_event_history that help assess the calls
    in event data"""
    for customer in customer_list:
        out_number = find_customer_by_number(call.src_number,
                                             customer_list)
        in_number = find_customer_by_number(call.dst_number,
                                            customer_list)
        if customer == out_number:
            customer.make_call(call)
        if customer == in_number:
...
def process_event_history(log: Dict[str, List[Dict]],
    ...
    for event_data in log['events']:
        if event_data["type"] == "call":
            calling_date = datetime.datetime.strptime(event_data['time'],
                                                      "%Y-%m-%d %H:%M:%S")
            calling_month = calling_date.month
            calls = Call(event_data['src_number'],
            ...
            if calling_month != billing_month:
                new_month(customer_list, calling_month, calling_date.year)
                billing_month = calling_month
            helper(customer_list, calls)
