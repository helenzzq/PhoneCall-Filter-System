...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        calling = list()
        num = []
        if str.isdigit(filter_string) and len(filter_string) == 4:
            ids = int(filter_string)
            for cut in customers:
                if ids == cut.get_id():
                    num = cut.get_phone_numbers()
            if len(num) == 0:
                return data
        else:
            return data

        if str.isdigit(filter_string) and num != []:
            for calls in data:
                if calls.dst_number in num or calls.src_number in num:
                    calling.append(calls)
            return calling
        else:
            return data
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if len(filter_string) <= 1:
            return data
        elif not str.isdigit(filter_string[1:]):
            return data
        elif filter_string[0] == 'L':
            time.sleep(0.02)
            if filter_string[0] == 'L' and filter_string[1:] == '0':
                return []
            for calls in data:
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if len(filter_string) == 0 or len(filters) != 4:
            return data
        else:
            for i in filters:
                try:
            ...
            lower_long = float(filters[0])
            lower_lat = float(filters[1])
            upper_long = float(filters[2])
            upper_lat = float(filters[3])
            if lower_long < map_min[0] or lower_lat < map_min[1] or upper_long\
                    > map_max[0] or upper_lat > map_max[1]:
                return data

            for calls in data:

                if calls.src_loc[0] >= lower_long and calls.src_loc[0] \
                    <= upper_long and calls.src_loc[1] \
                        >= lower_lat and calls.src_loc[1] <= upper_lat:
                    lst.append(calls)
                elif calls.dst_loc[0] >= lower_long and calls.dst_loc[0] \
                    <= upper_long and calls.dst_loc[1] \
                        >= lower_lat and calls.dst_loc[1] <= upper_lat:
                    lst.append(calls)
            return lst
